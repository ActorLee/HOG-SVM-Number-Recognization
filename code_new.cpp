...
int  train() {
	...
	svm->save("D:\\datasets\\number\\Detect.xml");
	Mat test(1, hog_dim, CV_32FC1);
	Mat img2 = imread(test2,0);
	Mat img3 = norm(image_processing_mat(img2));
	vector<float> des= get_hog(img3, w, h, blocksize_w, blocksize_h, b_stride_w, b_stride_h, cellsize_w, cellsize_h, nbins);
	for (vector<float>::size_type j = 0; j <=hog_dim - 1; ++j)
	{
		test.at<float>(0, j) = des[j];
	}
...
alltime += double(endTime - startTime);
	...
	Mat img = imread(test2,0);

	Mat test = norm(img);
	cout << detect_mat(test) << endl;
